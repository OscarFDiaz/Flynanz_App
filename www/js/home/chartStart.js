...
function loadChartData(expenseData) {
  ...
  let entry = false;
  let expenseCanvas = document.getElementById('expensesChart');

  if (expenseData.datasets.length > 0) {
    if (!expenses) {
      expenseData.labels.push(getLang('home_chart_noexpenses'));
      ...
      if (!entry) {
        expenseData.labels.push(getLang('home_chart_noexpenses_active'));
